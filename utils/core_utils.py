...
def find_func(model_name: str):
    ...
    # print(model_name)
    if model_name in ['mean_mil', 'max_mil', 'att_mil','trans_mil', 's4model','mamba_mil', 'psmil', 'rrt', 'clam_sb']:
...
def train(datasets, cur, args):
    ...
    elif args.model_type == 'psmil':
        from models.PSMIL import PSMIL
        model = PSMIL(in_dim = args.in_dim, n_classes=args.n_classes, dropout=args.drop_out, act='relu', layer = args.layer_number)
    else:
